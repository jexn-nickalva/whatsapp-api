...
app.get('/send-message', async (req, res) => {
  const destino = phoneNumberFormatter(req.query.destino);
  const message = req.query.message;

  const isRegisteredNumber = await checkRegisteredNumber(destino);

  if (!isRegisteredNumber) {
    return res.status(422).json({
      status: false,
      message: 'The destino is not registered'
    });
  }

  client.sendMessage(destino, message)
    .then(response => {
      res.status(200).json({
        status: true,
        response: response
